...
### Fixed — Python: overlapping face holes triangulated to an arbitrary, undefined triangle count (#285)

Root-caused the ".NET vs Python triangle/vertex-count divergence on a real file" item: a real fixture (`Untitled.skp`) has faces with two circular holes close enough together (centers 0.33" apart, radius 0.69" each — evidently one slotted/oval cutout recorded as two overlapping full circles) that they genuinely overlap. Feeding two overlapping rings to a hole-based earcut triangulator is undefined - there's no single well-defined triangulation of self-intersecting boundary input, so this project's own `mapbox_earcut` call and the independently-implemented .NET port each picked *some* triangle count for it, never the same one. It was never a case of one implementation needing to copy the other's number; neither was computing anything well-defined. `build_scene()`/`buildInstancedScene()` now detect real hole-hole overlap (a shared boundary edge/point - a normal, common pattern - does not count; only genuine shared area does) and replace the overlapping holes with the boundary of their union before triangulating, restoring a normal, well-defined hole shape. Verified against the geometric invariant that a valid triangulation of a polygon with h holes and n total boundary vertices always has exactly n - 2 + 2h triangles: the real fixture's affected faces (4-vertex outer + a 28-vertex merged hole boundary, 1 hole) now produce 4 + 28 - 2 + 2×1 = 32 triangles, exactly matching the fix's actual output. A no-op for the overwhelming common case of genuinely disjoint holes (confirmed: every other real fixture's scene-bake output is byte-for-byte unchanged). Not yet ported to TypeScript/.NET/Dart/C++ - each has its own independent earcut wrapper needing the same overlap/union preprocessing; tracked in [issue #285](https://github.com/iamahsanmehmood/openskp/issues/285).
