...
Two real models, generated by an AI coding agent from a natural-language
prompt, using nothing but the raw `openskp.create()` API — no primitives
library, no hand-authored geometry:

### Mid-century dining chair + accent table

9 component definitions, 38 meshes, 3 colored layers, 4 materials
(Walnut, Oak, Charcoal Linen, Brass). Demonstrates multiple materials on
one part (top vs. side vs. bottom face), reusable component definitions
(4 identical legs instanced, not hand-duplicated), a *nested* component
definition (a backrest assembly containing its own frame + cushion
sub-parts), custom attribute dictionaries, and genuine circular faces
(brass foot glides) — not polygon approximations.
...
1 top-level definition, 7 meshes, 3 layers, 6 materials including a
translucent glass material. Rectangular door/window openings cut via
`holes=` (a real hole loop in the same face, not a separate geometry
hack), a genuine circular accent window (`add_circle`), colored
Walls/Roof layers, and a foundation + porch step.

> **Screenshots pending.** Both models were reviewed in real SketchUp
> during development and confirmed correct (clean materials, no
> geometry defects), but that review didn't leave behind saved image
> files — this section is due an update with actual renders. The
> source `.py`/`.skp`/`.glb` files these numbers come from are real and
> reproducible today; ask if you'd like a copy.
...
The workflow that produced both showcase models: describe the object,
let the agent write and run `openskp.create()` code, open the result in
