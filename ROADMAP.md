...
- Reading a `.frag` file back (`from_fragments`/`read`, TypeScript's
  `fromFragments()`, .NET's `FragmentsExport.FromFragments()`) —
  **Python, TypeScript, and .NET done.** Dart/C++ can write Fragments but
  not read one back.
